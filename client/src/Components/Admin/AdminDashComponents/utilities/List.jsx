...
const List = (props) => {  
  ...
  const validateUser = async () => {
    try{
        if(window.ethereum){
            ...
            await VotingContract.validateUser(props.id).then(
              (log)=>{
  ...
  return (  
    <>
    <div className="min-w-[420px] bg-white p-4 rounded-2xl shadow-2xl mb-8 gap-6 flex flex-col">
      <div className="grid grid-cols-5 items-center justify-items-center gap-4">
        ...
        <div>
          <span className="bg-blue-100 text-cyan-800 py-1 px-3 rounded-full font-medium break-all flex items-center text-center">{props.communityID}</span>
        </div>
        <div>
        ...
        <div>
          {props.flag?<span className="bg-green-100 text-green-800 py-1 px-3 rounded-full font-medium">Verified</span>:<button className="bg-red-100 text-red-800 py-1 px-3 rounded-full font-medium" onClick={()=>{validateUser()}}>Verify</button>}
        </div>
      </div>
    </div>
