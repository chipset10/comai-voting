    ...
    const removeCandidate=async ()=>{
        try{
            if(window.ethereum){
                ...
                await VotingContract.mapCandidate(props.regId.toNumber(),props.proposalID.toNumber(),props.id.toNumber()).then(
                    (log)=>{
...
const Child=(props)=>{
    return (
        <>
            {props.data.map(item=>(<ShowList data={item.name} regId={item.regId} id={item.id} proposalID={props.proposalID} func={props.func} key={item.id}/>))}
        </>
    )
}
            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-gradient-to-r from-[#00374a] to-[#51182d] outline-none focus:outline-none">
                ...
                <div className="grid grid-cols-2 relative p-6 flex-auto">
                  <Child data={props.data} proposalID={props.proposalID} func={props.func}/>
                </div>
