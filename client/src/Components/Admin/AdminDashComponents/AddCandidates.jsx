...
const AddCandidates = () => {
  const [proposalData,setData]=useState([])
  ...
  const getProposalList = async () => {
    try{
        if(window.ethereum){
            ...
            await VotingContract.getProposals(false).then((proposal)=>{
              setData(proposal)
            })
        }
        else{
      ...
      if(selected.length!=0){
        try{
          if(window.ethereum){
              ...
              await VotingContract.addCandidate(candidateName,candidateRegNo,selected.id.toNumber()).then((log)=>{
  ...
  return (
    ...
    <section className="mt-6 gap-8">
        ...
        <div className='w-full mx-auto rounded-lg bg-primary-100 p-8 px-8'>
                <div className='flex flex-col text-[#e4d7ff] py-2'>
                <div className='grid items-center grid-cols-4 gap-4'>
                    <label className='grid justify-items-center font-bold text-xl text-white'>Select Proposal</label>
                    <div className='col-span-2' key="9087">    
                    <div>{chk && <Child dat={proposalData} func={handleCallback}/>}</div>
                    </div>
                    <div>
    ...
    <section className='mt-6 gap-8'>
        ...
        {flg?<>
          <form id='form' onSubmit={handleSubmit(onSubmit)} className='w-full mx-auto rounded-lg bg-primary-100 p-8 px-8'>
            ...
            </form> </>:console.log("No Proposal Selected") 
          } 
    </section>
    </div>
  )
