  ...
  const setWinner=async (id,data)=>{
    try{
      if(window.ethereum){
          ...
          await VotingContract.setTieWinner(id,data).then((log)=>{
        ...
        {props.tieFlg?(
            ...
            <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none backdrop-blur-md" onClick={(e)=>handleClick(e,props.setTieFlg)}>
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-gradient-to-r from-[#1e204b] to-[#1c0639] outline-none focus:outline-none">
                    ...
                    {selected!=null?<button className="bg-teal-500 rounded-xl p-2 text-slate-800 font-bold text-xl shadow-md shadow-gray-900 " onClick={()=>setWinner(props.id,selected)}>Confirm Winner Sproposal</button>:
  ...
  const getWinner=async(id)=>{
    try{
      if(window.ethereum){
          ...
          await VotingContract.getWinner(id).then((data)=>{
            if(data.length==4){
              if(data.name=="null"){
                setWinner("null")
              }
  ...
  const findWinner=async(id)=>{
    try{
      if(window.ethereum){
          ...
          await VotingContract.findWinner(id).then(async (data)=>{
        ...
        {flg && candidates.map((item,index)=>(
            ...
            {item.regId!=0 && 
                <>
                    ...
                    <span className="text-blue-100 font-bold text-md text-center">{item.regId.toNumber()}</span>
        ...
        <div className='grid gap-5 md:grid-cols-2 xl:grid-cols-3'>
                    {
                        props.proposals.map((item,index)=>(
                            <Fragment key={index}>
                            {item.totalVote>0?
                                <div className="bg-gradient-to-l from-slate-900 to-slate-800 border-2 border-gray-900 p-6 rounded-2xl gap-6 h-full flex flex-col justify-between">
                                    ...
                                    <div className="bg-gradient-to-l from-black via-gray-900 border-2 border-gray-900 p-4 rounded-2xl flex flex-col h-[30vh]">    
                                        <Child1 id={item.id}/>
                                    </div>
                                    {item.openForVotes==true?<span className="font-bold text-xl text-center text-green-400">Voting in Progress</span>:
                                      <Child2 id={item.id} />
                                    }
                                </div>:null}
                            </Fragment>
...
const ViewResults = () => {
    ...
    const getUserProposals=async()=>{
        try{
          if(window.ethereum){
              ...
              await VotingContract.getProposals(false).then((data)=>{
                setProposals(data)
                setFlg(true)
              })
            }
  ...
  return (
    <div className='mt-8 gap-8'>
        ...
        {flag?<div className='mt-4 rounded-2xl p-8 bg-white shadow-2xl overflow-y-auto scrollbar h-[70vh]'>
            <Child proposals={proposals}/>
        </div>:null}
    </div>
  )
}
