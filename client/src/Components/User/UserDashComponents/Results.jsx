        ...
        {flg && candidates.map((item,index)=>(
            ...
            {item.regId!=0 && 
                <>
                    ...
                    <span className="text-blue-100 font-bold text-xl text-center">{item.regId.toNumber()}</span>
        ...
        <div className='grid gap-5 md:grid-cols-2 xl:grid-cols-3'>
                    {
                        props.proposals.map((item,index)=>(
                            <Fragment key={index}>
                            {item.totalVote>0?
                                <div className="bg-gradient-to-l from-slate-900 to-slate-800 border-2 border-gray-900 p-5 rounded-2xl gap-6 h-full flex flex-col justify-between">
                                    ...
                                    <div className="bg-gradient-to-l from-indigo-600 via-cyan-700 to-teal-800 border-2 border-gray-900 p-4 rounded-2xl flex flex-col h-[30vh]">    
                                        <Child1 id={item.id}/>
                                    </div>
                                    {item.openForVotes==true?<span className="font-bold text-xl text-center blink">Voting in Progress</span>:
                                        <ResChild id={item.id}/>
                                    }
                                </div>:null}
                            </Fragment>
    ...
    const getUserProposals=async()=>{
        try{
          if(window.ethereum){
              ...
              await VotingContract.getAllMappedProposals(user.id).then((data)=>{
                setProposals(data)
                if(data.length>0)
                    setFlg(true)
              })
