        ...
        <>
            {props.data.map((item)=>
                ...
                <label htmlFor={item.id} className="flex cursor-pointer items-center gap-4 rounded-xl bg-indigo-100 bg-opacity-90 p-4 shadow-xl backdrop-blur-2xl transition border-2 border-current hover:bg-opacity-75 peer-checked:bg-gradient-to-r from-slate-900 to-teal-900 peer-checked:border-cyan-400 peer-checked:text-white">
                    ...
                    {item.regId!=0?<span className="text-sm opacity-80">Registration Number: {item.regId.toNumber()}</span>:
...
const VotingModal=(props)=>{
    ...
    const castUserVote=async(userID,proposalID,candidateID)=>{
        setChk(true)
        try{
            if(window.ethereum){
                ...
                await VotingContract.castVote(userID,proposalID,candidateID).then((log)=>{
        ...
        {props.showModal?(
            ...
            <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none backdrop-blur-md" onClick={(e)=>handleClick(e,props.setShowModal)}>
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full outline-none focus:outline-none">
                    ...
                    {!chk?selected!=null?<button className="bg-teal-400 rounded-xl p-2 text-white font-bold text-xl shadow-md shadow-gray-900 " onClick={()=>castUserVote(props.userID.toNumber(),props.proposalID.toNumber(),selected.id.toNumber())}>Confirm Proposal</button>:
...
const VoteCast = () => {
    ...
    const [proposalModalID,setProposalModalID]=useState(0)
    ...
    const castVote=async (proposalID)=>{
        await getMappedCandidates(proposalID).then(()=>{
    ...
    const getMappedCandidates=async(id)=>{
        try{
            if(window.ethereum){
                ...
                await VotingContract.getCandidates(id).then((data)=>{
                  setCandidates(data)
                })
              }
            else{
        ...
        try{
            if(window.ethereum){
                ...
                proposals.map(async(item,index)=>{
                    await VotingContract.checkVote(user.id,item.id.toNumber()).then((res)=>{
                        if(buttonFlg.length<proposals.length)
    ...
    const getUserProposals=async()=>{
        try{
          if(window.ethereum){
              ...
              await VotingContract.getMappedProposals(user.id).then((data)=>{
                setProposals(data)
              })
            }
          else{
                ...
                <div className='grid gap-5 md:grid-cols-2 lg:grid-cols-3'>
                    {
                        proposals.map((item,index)=>(
                            <div key={item.id} className='rounded-2xl bg-gradient-to-r from-[#6EE7B7] via-[#3B82F6] to-[#9333EA] p-[3px] shadow-lg shadow-gray-700 h-60'>
                                <div className="bg-gradient-to-l from-slate-900 to-slate-800 p-6 rounded-2xl gap-6 h-full flex flex-col justify-between">
                                    ...
                                    {showModal && proposalModalID===item.id.toNumber()?<VotingModal showModal={showModal} setShowModal={setShowModal} setProposalModalID={setProposalModalID} data={candidates.slice().reverse()} proposalID={item.id} userID={user.id}/>:null}
                                </div>
                            </div>
                        ))
                    }
