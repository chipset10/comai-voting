    ...
    const getProposalList = async () => {
        try{
            if(window.ethereum){
                ...
                await VotingContract.getMappedProposals(user.id.toNumber()).then((proposals)=>{
                  setData(proposals)
                  proposals.map(async(item,index)=>{
                    await VotingContract.checkVote(user.id,item.id.toNumber()).then((res)=>{
                      if(res){
                        if(voteCnt<=proposals.length){
                ...
                await VotingContract.getPolls(user.id.toNumber()).then((data)=>{
                  setHistory(data)
                  setFlg(true)
                })
            }
        ...
        return (
            <div className="mt-6 p-4 rounded-2xl">
                <div className="grid grid-rows-2 gap-6 sm:grid-cols-1 md:grid-cols-2">
                    <div className="row-span-2 p-8 rounded-2xl flex flex-col shadow-2xl gap-6 border-1 border-slate-900">
                        <div className="grid grid-cols-2 gap-y-4 items-center">
                            <h1 className="text-white text-xl font-bold grid justify-items-center">Total Active Proposals</h1>
                            ...
                            <div className="col-span-2 p-4 rounded-2xl shadow-2xl mt-2 gap-6 flex flex-col border-1 border-slate-900">
                                <Pie className="lg:max-h-72" data={linedata}></Pie>
                            </div>
                        </div>
                    </div>
                    <div className=" grid items-center p-8 rounded-2xl shadow-2xl gap-6 border-1 border-slate-900">
                        <div className="grid grid-cols-2 gap-y-4 items-center">
                    ...
                    <div className=" grid items-center justify-center p-6 rounded-2xl shadow-2xl gap-6 border-1 border-slate-900">
                      ...
                      <div className="grid grid-cols-2 gap-4 items-center">
                              ...
                              <h1 className="text-white text-lg font-bold grid text-center">CommunityID Number</h1>
                              <h1 className="text-[#9de4f8] text-lg font-bold text-start">{user.communityID}</h1>
