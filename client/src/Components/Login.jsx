...
const Popup=(props)=>{
    ...
    const checkOTP=async(token,secret)=>{
        ...
        await VotingContract.getUserDetails().then(async(user)=>{
            if(secret!==user.authCode){
    ...
    const genPass=async()=>{
        ...
        await VotingContract.setPass(pwd).then(()=>{
            setNewPass(pwd)
        })
    }
  return (
            ...
            <div className="relative w-2/4 my-6 mx-auto">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                    <div className=" mr-10">
                        <h3 className="text-3xl font-semibold text-white">2FA (One Time Password)</h3>
                    </div>
                ...
                <div className="flex items-center justify-center p-3 border-t border-solid border-slate-200 rounded-b">
                    {newPass!==''?<h1 className='bg-teal-500 font-bold text-lg text-slate-900 rounded-lg py-2 px-4'>Success!!</h1>:pass==true?<button id="otp-button" className='bg-teal-400 font-bold text-lg text-sky-800 rounded-lg py-2 px-4' onClick={()=>genPass()}>Generate New Password</button>:otp.length==6 && secretKey.length==16?<button id="otp-button" className='bg-teal-400 font-bold text-lg text-sky-800 rounded-lg py-2 px-4' onClick={()=>checkOTP(otp,secretKey)}>Verify</button>:<h1 className='bg-gray-600 font-bold text-lg text-gray-400 rounded-lg py-2 px-4'>Verify</h1>}
                </div>
              </div>
            </div>
          </div>
...
const OtpModal=(props)=>{
    ...
    const checkOTP=async(token)=>{
        ...
        await VotingContract.getUserDetails().then(async(user)=>{
            let tmp=verify2Fa(token,user.authCode)
            let status=tmp
            if(status==false){
                document.getElementById('error-otp').innerText="Error: Incorrect OTP"
                setOTP('')
                ...
                if(res==true){
                    ...
                    alert("Invalid CommunityID/Login Key")
                else
...
const Login = () => {
    ...
    const signinUser = async () => {
        try{
            if(communityID=="000000000000"){
                alert("This Community ID is invalid!")
                ...
                let res=await VotingContract.loginUser(loginKey,communityID)
                await VotingContract.getUserDetails().then((user)=>{
                    setUser(user)
    ...
    <div className="flex items-center justify-center text-white custom-img">
        <div className='h-screen flex flex-col justify-center w-full rounded-lg'>
            <form id='form' onSubmit={handleSubmit(onSubmit)} className='max-w-[455px] w-full mx-auto rounded-lg p-8 px-8  bg-[#3083b1]'>
                ...
                <div className='flex flex-col text-white mt-1 py-2'>
                    ...
                    <input className='rounded-lg bg-gray-700 mt-2 p-2 focus:border-blue-500 focus:bg-gray-800 focus:outline-none' type="number" {...register("communityID", { required: true, minLength: 12, maxLength:12})} onKeyDown={(e)=>(e.key==='e'||e.key==='.') && e.preventDefault()} onChange={(e)=>setCommunityID(e.target.value)}/> 
                </div>
                <div className='flex flex-col text-white mt-1 py-2'>
                    <div className='flex justify-between'>
                        <label>Password</label>
                        <h1 className=' text-black'>{errors.loginKey?.type === "required" && "Login Key is required"}</h1>
                    </div>
                ...
                <div className='flex flex-col text-white pb-2'>
                    ...
                    <input id='walletfield' className='p-2 rounded-lg bg-gray-700 mt-2 focus:border-blue-500 focus:bg-gray-800 focus:outline-none text-green-700 font-bold' type="text" readOnly={true} placeholder='0x0000000000000000000000000000000000000000'/>
                </div>
                <div>
