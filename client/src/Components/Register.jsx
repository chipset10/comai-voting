            ...
            <div className="relative w-2/4 my-6 mx-auto">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                    <div className=" mr-10">
                        <h3 className="text-3xl font-semibold text-white">2FA (One Time Password) Setup</h3>
                    </div>
                </div>
                {!otpModal?
                    <>
                        ...
                        <h1 className='text-xl font-bold pb-6'>2FA Verification Key : {props.secret}</h1>
                        <img className='pb-6' src={props.qrData}/>
                        <h1 className='text-md text-white font-bold pb-6'>Scan QR or enter key into Authenticator App! Once added, click on Next</h1>
                        <h1 className='text-lg font-bold pb-6'>Note: Save your password and key. This will not be displayed again!</h1>
                        </>}
                        </div>
                        <div className="flex items-center justify-center p-3 border-t border-solid border-slate-200 rounded-b">
                            <button className='bg-primary-100 font-bold text-lg text-white rounded-lg p-2' onClick={()=>handleClick()}>Next</button>
                        </div>
                    </>:
                    <>
                    {!otpsuccess?<>
                        ...
                        <div className="flex items-center justify-center p-3 border-t border-solid border-slate-200 rounded-b">
                            {otp.length==6?<button id="otp-button" className='bg-teal-400 font-bold text-lg text-sky-800 rounded-lg py-2 px-4' onClick={()=>checkOTP(otp,props.secret)}>Verify OTP</button>:<h1 className='bg-gray-600 font-bold text-lg text-gray-400 rounded-lg py-2 px-4'>Verify OTP</h1>}
                        </div></>:
                        <>
                        <div className="pt-6 px-6 pb-2 flex flex-col items-center justify-center">
                            <h1 className='p-2 text-white font-bold text-xl'>Confirming Registration!</h1>
                            <Loader/>
                        </div>
                        <div className="flex items-center justify-center p-3 border-t border-solid border-slate-200 rounded-b">
                        </div></>
...
const Register = () => {
    ...
    const checkUser = async () =>{
        try{
            ...
            if(window){
                ...
                // console.log(await VotingContract.getSender())
                return await VotingContract.isUserRegistered()
            }
            else{
    ...
    const addUser = async () => {
        try{
            if(window.ethereum){
                ...
                await VotingContract.addUser(communityID,name,email,password,cid,sec,false).then((log)=>{
                    provider.waitForTransaction(log.hash,1).then((receipt)=>{
                        if(receipt){
                            if(receipt.status==1){
                                // alert("Your login key is: "+password)
                                navigate('/login')
                            }
                        }
                    })
    ...
    useEffect(()=>{
        ...
        {
            console.log("Already Registered!")
            alert("You have connected the wallet and registered already. Login now!")
            navigate('/login')
        }})
    },[account])
    ...
    return (
    <div className="flex items-center justify-center text-white custom-img">
        <div className='h-screen flex flex-col justify-center w-full rounded-lg'>
            <form id='form' onSubmit={handleSubmit(onSubmit)} className='max-w-[512px] w-full mx-auto rounded-lg p-8  bg-[#3083b1]'>
                <h2 className='text-4xl dark:text-white font-bold text-center'>Register</h2>
                <div className='flex flex-col text-white mt-1 py-2'>
                    <div className='flex justify-between'>
                        <label>Name</label>
                        <h1 className=' text-black'>{errors.name?.type === "required" && "Name is required"}</h1>
                    </div>
                ...
                <div className='flex flex-col text-white mt-1 py-2'>
                    <div className='flex justify-between'>
                        ...
                        <h1 className=' text-black'>{errors.communityID?.type === "required" && "CommunityID number is required"}
                        {errors.communityID?.type === "minLength" && "CommunityID number cannot be less than 12 digits"}
                        {errors.communityID?.type === "maxLength" && "CommunityID number cannot be more than 12 digits"}</h1>
                    </div>
                    <input className='rounded-lg bg-gray-700 mt-1 p-2 focus:border-blue-500 focus:bg-gray-800 focus:outline-none' type="number" {...register("communityID", { required: true, minLength: 12, maxLength:12})} onKeyDown={(e)=>(e.key==='e'||e.key==='.') && e.preventDefault()} onChange={(e)=>setCommunityID(e.target.value)}/>
                </div>
                <div id='email' className='flex flex-col text-white mt-1 py-2'>
                    <div className='flex justify-between'>
                        <label>Email ID</label>
                        <h1 className=' text-black'>{errors.email?.type === "required" && "Email is required"}{errors.email?.type === "pattern" && "Entered email is in wrong format"}</h1>
                    </div>
                ...
                <div id='fileproof' className='flex flex-col text-white mt-1 py-2'>
                    <div className='flex justify-between'>
                ...
                <div className='flex flex-col text-white mt-1 py-2'>
                    ...
                    <input id='walletfield' className='p-2 rounded-lg bg-gray-700 mt-1 focus:border-blue-500 focus:bg-gray-800 focus:outline-none text-green-700 font-bold' type="text" readOnly={true} placeholder='0x0000000000000000000000000000000000000000'/>
                </div>
                <div>
